...
describe.skip('GET /api/operators', async () => {
  let pool: Pool;
  ...
  describe('pagination', async () => {
    it('trie par last_activity DESC par défaut', async () => {
      await service.upsertOperator('op-old', 1000);
      await service.upsertOperator('op-new', 3000);
      await service.upsertOperator('op-mid', 2000);
    ...
    it('pagine avec limit+offset', async () => {
      for (let i = 0; i < 25; i++) {
        await service.upsertOperator(`op-${String(i).padStart(3, '0')}`, 1000 + i);
      }
      const p1 = await request(app).get('/api/operators?limit=10&offset=0');
      ...
      const p3 = await request(app).get('/api/operators?limit=10&offset=20');
    ...
    it('limit default=20, offset default=0', async () => {
      for (let i = 0; i < 30; i++) {
        await service.upsertOperator(`op-${i}`, 1000 + i);
      }
      const res = await request(app).get('/api/operators');
  ...
  describe('filtre status', async () => {
    it('filtre status=verified', async () => {
      await service.upsertOperator('op-v1', 1000);
      await service.upsertOperator('op-v2', 2000);
      await service.upsertOperator('op-p1', 3000);
      await operators.updateVerification('op-v1', 2, 'verified');
      await operators.updateVerification('op-v2', 2, 'verified');

      const res = await request(app).get('/api/operators?status=verified');
      expect(res.status).toBe(200);
    ...
    it('filtre status=pending', async () => {
      await service.upsertOperator('op-p1', 1000);
      await service.upsertOperator('op-p2', 2000);
      await service.upsertOperator('op-v1', 3000);
      await operators.updateVerification('op-v1', 2, 'verified');

      const res = await request(app).get('/api/operators?status=pending');
      expect(res.body.data).toHaveLength(2);
      expect(res.body.data.every((r: { status: string }) => r.status === 'pending')).toBe(true);
    });

    it('filtre status=rejected', async () => {
      await service.upsertOperator('op-r1', 1000);
      await operators.updateVerification('op-r1', 0, 'rejected');

      const res = await request(app).get('/api/operators?status=rejected');
  ...
  describe('validation', async () => {
    it('400 sur status inconnu', async () => {
      const res = await request(app).get('/api/operators?status=zombie');
      expect(res.status).toBe(400);
      expect(res.body.error.code).toBe('VALIDATION_ERROR');
    });

    it('400 sur limit > 100', async () => {
      const res = await request(app).get('/api/operators?limit=500');
      expect(res.status).toBe(400);
    });

    it('400 sur offset négatif', async () => {
      const res = await request(app).get('/api/operators?offset=-10');
      expect(res.status).toBe(400);
    });

    it('400 sur limit=0', async () => {
      const res = await request(app).get('/api/operators?limit=0');
      expect(res.status).toBe(400);
    });
  });
  ...
  describe('fields', async () => {
    it('expose operator_id, status, verification_score, timestamps', async () => {
      await service.upsertOperator('op-fields', 1000);
      await operators.updateVerification('op-fields', 2, 'verified');
