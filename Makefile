...
deploy:
	...
	ssh $(SATRANK_HOST) "chown -R root:root $(REMOTE_DIR) && chmod 600 $(REMOTE_DIR)/.env.production 2>/dev/null && chown 1001:1001 $(REMOTE_DIR)/probe-pay.macaroon $(REMOTE_DIR)/invoice.macaroon $(REMOTE_DIR)/fulfill.macaroon $(REMOTE_DIR)/onchain.macaroon 2>/dev/null || true"
