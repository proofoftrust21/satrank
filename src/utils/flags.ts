...
export function computeBaseFlags(agent: Agent, delta: DeltaInput, now: number): VerdictFlag[] {
  ...
  // p_success scale (0..1) — calibrated at p7/p93 of 7d delta distribution
  // (scripts/analyzeDeltaDistribution.ts). Same population coverage as the
  // previous -10/+15 composite thresholds.
  if (delta.delta7d !== null && delta.delta7d < -0.10) flags.push('rapid_decline');
  if (delta.delta7d !== null && delta.delta7d > 0.15) flags.push('rapid_rise');
