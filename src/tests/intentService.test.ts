...
function seedEndpoint(db: Database.Database, serviceRepo: ServiceEndpointRepository, agentRepo: AgentRepository, f: Fixture): void {
  agentRepo.insert(makeAgent(f.hash));
  serviceRepo.upsert(f.hash, f.url, f.httpStatus ?? 200, f.latencyMs ?? 200, '402index');
  serviceRepo.updateMetadata(f.url, {
    name: f.name,
    description: f.description ?? null,
    category: f.category,
    provider: f.provider ?? null,
  });
  serviceRepo.updatePrice(f.url, f.priceSats);

  // Force check/success counts if provided
  if (f.checkCount != null) {
    db.prepare('UPDATE service_endpoints SET check_count = ?, success_count = ? WHERE url = ?')
      .run(f.checkCount, f.successCount ?? f.checkCount, f.url);
  }

  if (f.seedSafe) {
    seedSafeBayesianObservations(db, f.hash, { now: NOW });
  }
}

function buildService(db: Database.Database): IntentService {
...
describe('IntentService', () => {
  ...
  describe('listCategories', () => {
    it('retourne les catégories avec endpoint_count et active_count', () => {
  ...
  describe('resolveIntent', () => {
    it('retourne les candidats triés par p_success DESC', () => {
      ...
      const svc = buildService(db);
      const res = svc.resolveIntent({ category: 'data/finance' }, 5);
    ...
    it('filtre budget_sats : exclut les endpoints au-dessus du budget', () => {
      seedEndpoint(db, serviceRepo, agentRepo, {
      ...
      const svc = buildService(db);
      const res = svc.resolveIntent({ category: 'tools', budget_sats: 10 }, 5);
      expect(res.candidates.map(c => c.endpoint_url)).toEqual(['https://cheap.example/x']);
      expect(res.meta.total_matched).toBe(1);
    });

    it('filtre keywords AND : chaque keyword doit matcher', () => {
      seedEndpoint(db, serviceRepo, agentRepo, {
      ...
      const svc = buildService(db);
      const res = svc.resolveIntent({ category: 'data', keywords: ['forecast', 'paris'] }, 5);
      expect(res.candidates.map(c => c.endpoint_url)).toEqual(['https://x.example/paris']);
    });

    it('limit par défaut 5, clamp à 20', () => {
      for (let i = 0; i < 30; i++) {
        seedEndpoint(db, serviceRepo, agentRepo, {
      ...
      const svc = buildService(db);
      expect(svc.resolveIntent({ category: 'data' }, undefined).candidates.length).toBe(5);
      expect(svc.resolveIntent({ category: 'data' }, 10).candidates.length).toBe(10);
      expect(svc.resolveIntent({ category: 'data' }, 99).candidates.length).toBe(INTENT_LIMIT_MAX);
      expect(svc.resolveIntent({ category: 'data' }, -5).candidates.length).toBe(1);
    });

    it('strictness=relaxed quand aucun candidat SAFE', () => {
      seedEndpoint(db, serviceRepo, agentRepo, {
      ...
      const svc = buildService(db);
      const res = svc.resolveIntent({ category: 'tools' }, 5);
      expect(res.meta.strictness).toBe('relaxed');
      expect(res.meta.warnings).toContain('FALLBACK_RELAXED');
      expect(res.candidates).toHaveLength(1);
    });

    it('pool vide retourne candidates: [] avec strictness=degraded', () => {
      const svc = buildService(db);
      const res = svc.resolveIntent({ category: 'does-not-exist' }, 5);
    ...
    it('tri tertiaire sur price_sats ASC quand p_success + ci95_low égaux', () => {
    ...
    it('max_latency_ms filtre via median_latency_ms : table vide → tout rejeté', () => {
      ...
      const svc = buildService(db);
      const withFilter = svc.resolveIntent({ category: 'data', max_latency_ms: 500 }, 5);
      expect(withFilter.candidates).toEqual([]);
      expect(withFilter.meta.total_matched).toBe(0);

      const withoutFilter = svc.resolveIntent({ category: 'data' }, 5);
      expect(withoutFilter.candidates).toHaveLength(1);
    });

    it('candidat exclut RISKY même en fallback degraded', () => {
  ...
  describe('knownCategoryNames', () => {
    it('retourne un Set des catégories vivantes', () => {
      seedEndpoint(db, serviceRepo, agentRepo, {
      ...
      const svc = buildService(db);
      const names = svc.knownCategoryNames();
