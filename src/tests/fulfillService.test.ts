...
// fulfillService wraps the param with decode($1, 'hex'). Test pubkey MUST
// be valid 64-char hex.
const AGENT_HASH = 'a'.repeat(64);

async function seedAgentBalance(pool: Pool, agentPubkey: string, sats: number): Promise<void> {
  // Phase 12A audit fix — payment_hash is bytea, prod queries use decode().
  await pool.query(
