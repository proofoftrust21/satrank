...
export class ReportService {
   ...
   *    'intent' — this report closes out a prior paid target-query from the
  ...
  private async classifySource(
    ...
    try {
      ...
      logger.warn(
        ...
        'token_query_log lookup failed, falling back to source=report',
      );
      return 'report';
    }
  }
  ...
  async submit(input: ReportRequest): Promise<ReportResponse> {
    ...
    const doInsert = async (
      ...
      if (!existingTx) {
        ...
        // à l'ingestion via weightForSource. Les intents (token_query_log) ne sont
